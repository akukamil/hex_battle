...
var app, game_res, game, objects={}, LANG = 0, state="", game_tick=0, made_moves=0, game_id=0, my_turn=0, my_tile=1, connected = 1;
...
var mp_game = {
	...
	activate : async function (role, level) {
		...
		if (this.my_role === 'master') {
			my_turn = 1;			
			opp_tile = irnd(2,6);
			board.draw_board(levels[level], 1, opp_tile);
		} else {
			my_turn = 0;	
			opp_tile = irnd(2,6);
			board.draw_board(levels[level], 2, opp_tile);
		}
...
var sp_game = {
	...
	activate: async function() {
		...
		board.draw_board(levels[this.level_id], 1, opp_tile);
...
var board = {
	...
	draw_board : async function(level_data, my_level_tile_id, _opp_tile_id) {
		...
		objects.hex_cells.forEach((c,i)=>{		
			...
			let level_tile_id = level_data[i];				
			...
			if (level_tile_id > 0) {			
				...
				c.visible = c.icon.visible = true;		
				
				if (level_tile_id === my_level_tile_id)					
					c.player_id = 1;
				else					
					c.player_id = _opp_tile_id;
